...
// --- IKON BARU UNTUK KATEGORI (BEREALISTIK DAN CANTIK) ---
...
const IconCake = () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18c-2.305 0-4.408.867-6 2.292m0-14.25v14.25" /></svg>; // Icon RTE/Siap Masak (Buku resepi/Cake)
const IconCube = () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="m21 7.5-9-5.25L3 7.5m18 0-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9" /></svg>; // Icon Dry/Kering (Generic Cube)
...
const calculateDaysLeft = (expiryDateStr: string): number => {
  ...
  if (isNaN(expiryDate.getTime())) return 999; // Error tarikh
...
const categoryMap: {[key: string]: {label: string, icon: JSX.Element, color: string}} = {
  kering: {label: 'Kering (Dry)', icon: <IconCube />, color: 'amber'},
  basah: {label: 'Basah (Wet)', icon: <IconDroplet />, color: 'blue'},
  rte: {label: 'Siap Masak (Ready-to-Eat)', icon: <IconCake />, color: 'red'},
};
...
export default function Home() {
  ...
  const [category, setCategory] = useState('kering'); // Keadaan kategori baharu
  const [senaraiBarang, setSenaraiBarang] = useState<any[]>([]);
  const [activeFilter, setActiveFilter] = useState('semua'); // Penapisan tab jadual
  ...
  const tarikData = async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        // Susun tarikh luput paling dekat dulu
  ...
  const hantarData = async () => {
    ...
    const res = await fetch('/api/add-item', {
      ...
      body: JSON.stringify({ 
        ...
        expiry_date: date,
        location: location,
        category: category // Hantar kategori baharu
      }),
    });
    const data = await res.json();
    ...
    setDate('');
    setLocation('peti');
    setCategory('kering'); // Reset kategori
    tarikData();
  };
  ...
  const padamData = async (itemId: number) => {
    ...
    try {
      ...
      if (data.message) {
        alert(data.message);
        tarikData(); // Auto refresh jadual Lepas padam
      } else {
  ...
  // Kira baki hari untuk setiap barang
  ...
  // Kira jumlah untuk Dashboard Kad
  // updated logic: basah critical = basah items <3 days + rte items <3 days
  ...
  // updated logic: kering critical = kering items <=30 days
  const countKeringWarning = barangWithDays.filter(b => b.category === 'kering' && b.daysLeft >= 0 && b.daysLeft <= 30).length; // Luput sebulan
  const countSemua = senaraiBarang.length;

  // Tapis senarai barang untuk jadual berdasarkan tab yang aktif
  const filteredSenarai = useMemo(() => {
    switch (activeFilter) {
      case 'kering':
        return barangWithDays.filter(b => b.category === 'kering' && b.daysLeft >= 0 && b.daysLeft <= 30); // Penapisan Kering
      case 'basah':
        // Updated: filter only 'basah' category, remove '(<3 hari)' logic
        return barangWithDays.filter(b => b.category === 'basah'); // Semua barang basah
      case 'rte':
        // New tab logic: filter 'rte' category AND < 3 days
        return barangWithDays.filter(b => b.category === 'rte' && b.daysLeft >= 0 && b.daysLeft <= 3); // Siap Masak critical
      case 'expired':
        return barangWithDays.filter(b => b.daysLeft < 0); // Penapisan Sudah Luput
      default:
        return barangWithDays; // Semua barang
    }
  ...
  return (
    <main className="p-6 md:p-10 flex flex-col gap-6 max-w-5xl mx-auto bg-gray-50 min-h-screen">
      ...
      {/* FORM TAMBAH BARANG BARU (DENGAN MEDAN KATEGORI) */}
      <div className="flex flex-col gap-5 p-6 bg-white rounded-xl shadow-md">
        ...
        {/* --- MEDAN KATEGORI (RADIO BUTTON WITH ICONS) --- merujuk kepada `image_34.png` */}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-3 mt-2">
                {kategoriOptions.map(opt => {
                    ...
                    return (
                        <label key={opt} className={`flex items-center gap-3 border p-4 rounded-xl cursor-pointer transition-all ${category === opt ? `border-${k.color}-500 bg-${k.color}-50 ring-2 ring-${k.color}-200` : 'border-gray-200 bg-gray-50 hover:bg-white'}`}>
                            <input type="radio" name="category" value={opt} checked={category === opt} onChange={() => setCategory(opt)} className={`w-5 h-5 text-${k.color}-600`} />
                            <div className={`${category === opt ? `text-${k.color}-600` : 'text-gray-500'}`}>{k.icon}</div>
                            <div className={`${category === opt ? `text-${k.color}-800` : 'text-gray-700'} font-medium`}>{k.label}</div>
                        </label>
                    );
                })}
              </div>
      ...
      {/* SENARAI BARANG DENGAN TAB PENAPISAN JADUAL YNG DIKEMASKINI */}
      <div className="mt-5 p-6 bg-white rounded-xl shadow-md">
        ...
        {/* --- TAB PENAPISAN JADUAL --- merujuk kepada `image_35.png` */}
        <div className="flex border-b mb-6 overflow-x-auto">
          {[
            {key: 'semua', label: 'Semua'},
            {key: 'kering', label: 'Kering (<30 hari)'},
            {key: 'basah', label: 'Basah'}, // updated: removed (<3 hari)
            {key: 'rte', label: 'Siap Masak (Ready to Eat) (< 3 hari)'}, // new tab
        ...
        {/* JADUAL BARANG (DENGAN KOLUM KATEGORI) */}
        {/* JADUAL BARANG (DENGAN KOLUM KATEGORI) */}
              ...
              {filteredSenarai.map((barang, index) => {
                ...
                return (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 border-b">
                        <div className={`flex items-center gap-2 text-${k.color}-700 bg-${k.color}-50 px-3 py-1.5 rounded-full inline-flex font-semibold text-xs`}>
                            {k.icon}
                            {k.label}
                        </div>
                    </td>
